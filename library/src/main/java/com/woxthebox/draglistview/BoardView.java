...
public class BoardView extends HorizontalScrollView implements AutoScroller.AutoScrollListener {
    ...
    //TODO
    public void scrollToPosition(int column, int position) {
        scrollToColumn(column);

        if (column >= 0 && column <= mLists.size())
            scrollToPosition(mLists.get(column), position);
    }
        ...
        if (mCurrentRecyclerView != currentList) {
            ...
            if (mBoardCallback == null || mBoardCallback.canDropItemAtPosition(mDragStartColumn, mDragStartRow, newColumn, newPosition)) {
                ...
                if (item != null) {
                    ...
                    mDragItem.setOffset(((View) mCurrentRecyclerView.getParent().getParent()).getLeft(), mCurrentRecyclerView.getTop());
    ...
    private float getListTouchX(DragItemRecyclerView list) {
        return mTouchX + getScrollX() - ((View) list.getParent().getParent()).getLeft();
    }
    ...
    private DragItemRecyclerView getCurrentRecyclerView(float x) {
        for (DragItemRecyclerView list : mLists) {
            View parent = (View) list.getParent().getParent();
    ...
    private int getClosestSnapColumn() {
        ...
        for (int i = 0; i < mLists.size(); i++) {
            View listParent = (View) mLists.get(i).getParent().getParent();
...
//        View parent = (View) mLists.get(column).getParent();
        ...
        //TODO HotFixFor SwipeRefreshLayout
        View parent = (View) mLists.get(column).getParent().getParent();
    ...
    public DragItemRecyclerView addColumnList(final DragItemAdapter adapter, final View header, boolean hasFixedItemSize) {
        ...
        recyclerView.setDragItemListener(new DragItemRecyclerView.DragItemListener() {
            ...
            public void onDragStarted(int itemPosition, float x, float y) {
                ...
                mDragItem.setOffset(((View) mCurrentRecyclerView.getParent().getParent()).getX(), mCurrentRecyclerView.getY());
                if (mBoardListener != null) {
        ...
        layout.addView(initSwipeRefreshLayout(recyclerView));
        // layout.addView(recyclerView); // without SwipeRefreshLayout
    ...
    public interface ColumnsSwipeRefreshLayoutListener {
        void onColumnRefresh(SwipeRefreshLayout refreshLayout, int column);
    }

    private ColumnsSwipeRefreshLayoutListener mSwipeRefreshLayoutListener;

    public void setSwipeRefreshLayoutListener(ColumnsSwipeRefreshLayoutListener swipeRefreshLayoutListener) {
        mSwipeRefreshLayoutListener = swipeRefreshLayoutListener;
    }

    private SwipeRefreshLayout initSwipeRefreshLayout(final RecyclerView recyclerView) {
        //TODO
        final SwipeRefreshLayout swipeRefreshLayout = new SwipeRefreshLayout(getContext());
        swipeRefreshLayout.setLayoutParams(new LayoutParams(mColumnWidth, LayoutParams.MATCH_PARENT));
        swipeRefreshLayout.setColorSchemeColors(ContextCompat.getColor(getContext(), R.color.app_color)); //TODO COLOR
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                if (mSwipeRefreshLayoutListener != null) {
                    int column = mLists.indexOf(recyclerView);
                    mSwipeRefreshLayoutListener.onColumnRefresh(swipeRefreshLayout, column);
                }
            }
        });
